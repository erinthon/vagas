...
@Component({
  ...
  template: `
      ...
      <div class="form-card">
        ...
        <div class="form-grid">
          <div class="field">
            ...
            <select [(ngModel)]="form.moradorId" (ngModelChange)="onMoradorChange()">
          ...
          <div class="field">
            <label>Vaga *</label>
            <select [(ngModel)]="form.vagaId">
              <option value="">{{ form.moradorId ? 'Selecione...' : 'Selecione um morador primeiro' }}</option>
              @for (v of vagasDoMorador(); track v.id) {
                <option [value]="v.id">{{ v.numero }} — {{ v.tipo }}</option>
              }
            </select>
          </div>
    ...
    @if (carregando()) {
      ...
      <div class="table-wrap">
        <table>
          <thead>
            <tr><th>ID</th><th>Morador</th><th>Vaga</th><th>Início</th><th>Fim</th><th>Status</th><th>Ações</th></tr>
          </thead>
          <tbody>
            @for (o of items(); track o.id) {
              <tr>
                ...
                <td>{{ o.dataInicio }}</td>
                <td>{{ o.dataFim }}</td>
                <td><span class="tag" [class.ativa]="o.status === 'ATIVA'" [class.enc]="o.status === 'ENCERRADA'">{{ o.status }}</span></td>
                <td class="actions">
                  @if (o.status === 'ATIVA') {
                    <button class="btn-del" (click)="encerrar(o)">Encerrar</button>
                  }
                </td>
              </tr>
            }
  ...
  styles: [`
    ...
    .tag.enc { background: #f1f5f9; color: #64748b; }
...
export class AdminOfertas implements OnInit {
  ...
  vagasDoMorador() {
    if (!this.form.moradorId) return [];
    return this.vagas().filter(v => v.proprietario?.id === +this.form.moradorId);
  }

  ngOnInit() {
    forkJoin({ o: this.service.ofertas(), m: this.service.moradores(), v: this.service.vagas() }).subscribe({
      next: r => { this.items.set(r.o); this.moradores.set(r.m); this.vagas.set(r.v); this.carregando.set(false); },
      error: () => this.carregando.set(false)
    });
  }

  carregar() { this.service.ofertas().subscribe({ next: d => this.items.set(d) }); }

  abrirNovo() {
    if (this.mostrarForm()) { this.fecharForm(); return; }
    this.form = emptyForm(); this.erro.set(''); this.mostrarForm.set(true);
  }

  fecharForm() { this.mostrarForm.set(false); this.erro.set(''); }

  onMoradorChange() { this.form.vagaId = ''; }

  salvar() {
    if (!this.form.moradorId || !this.form.vagaId || !this.form.dataInicio || !this.form.dataFim) {
      this.erro.set('Preencha todos os campos obrigatórios.'); return;
    }
    this.salvando.set(true); this.erro.set('');
    const payload = { morador: { id: +this.form.moradorId }, vaga: { id: +this.form.vagaId }, dataInicio: this.form.dataInicio, dataFim: this.form.dataFim, observacao: this.form.observacao || null };
    this.service.criarOferta(payload).subscribe({
      next: () => { this.salvando.set(false); this.fecharForm(); this.carregar(); },
      error: err => { this.salvando.set(false); this.erro.set(err?.error?.message || 'Erro ao criar oferta.'); }
    });
  }

  encerrar(o: Oferta) {
    if (!confirm(`Encerrar oferta #${o.id}?`)) return;
    this.service.encerrarOferta(o.id!).subscribe({
      next: () => this.carregar(),
      error: err => alert(err?.error?.message || 'Erro ao encerrar.')
    });
  }
}

function emptyForm() { return { moradorId: '', vagaId: '', dataInicio: '', dataFim: '', observacao: '' }; }
