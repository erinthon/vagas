...
@Component({
  ...
  template: `
    ...
    @if (mostrarForm()) {
      <div class="form-card">
        <h2>{{ editando() ? 'Editar cargo #' + editando()!.id : 'Novo cargo' }}</h2>
        <div class="form-grid">
          ...
          @if (!editando()) {
            ...
            <div class="field">
              <label>Condomínio</label>
              <input [value]="editando()!.condominio?.nome ?? '—'" disabled />
            </div>
          }
        </div>
    ...
    @if (carregando()) {
      ...
      <div class="table-wrap">
        <table>
          <thead>
            <tr><th>ID</th><th>Nome</th><th>Condomínio</th><th>Ações</th></tr>
          </thead>
          <tbody>
            @for (c of items(); track c.id) {
              <tr>
                ...
                <td class="actions">
                  ...
                  <button class="btn-del" (click)="excluir(c)">Excluir</button>
                </td>
              </tr>
            }
          </tbody>
...
export class AdminCargos implements OnInit {
  ...
  editando    = signal<Cargo | null>(null);
  erro        = signal('');
  items       = signal<Cargo[]>([]);
  condominios = signal<Condominio[]>([]);

  form = { nome: '', condominioId: '' };

  ngOnInit() {
    forkJoin({ c: this.service.cargos(), cond: this.service.condominios() }).subscribe({
      next: r => { this.items.set(r.c); this.condominios.set(r.cond); this.carregando.set(false); },
      error: () => this.carregando.set(false)
    });
  }

  carregar() { this.service.cargos().subscribe({ next: d => this.items.set(d) }); }

  abrirNovo() {
    if (this.mostrarForm() && !this.editando()) { this.fecharForm(); return; }
    this.editando.set(null); this.form = { nome: '', condominioId: '' }; this.erro.set(''); this.mostrarForm.set(true);
  }

  editar(c: Cargo) {
    this.editando.set(c);
    this.form = { nome: c.nome, condominioId: String(c.condominio?.id ?? '') };
  ...
  salvar() {
    ...
    const condominioId = ed ? ed.condominio!.id! : +this.form.condominioId;
    const payload = { nome: this.form.nome, condominio: { id: condominioId } };
    const req$ = ed
      ? this.service.atualizarCargo(condominioId, ed.id!, payload)
  ...
  excluir(c: Cargo) {
    if (!confirm(`Excluir cargo "${c.nome}"?`)) return;
    this.service.excluirCargo(c.condominio!.id!, c.id!).subscribe({
      next: () => this.carregar(),
